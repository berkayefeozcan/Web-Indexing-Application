...
def clean_wordlist(wordlist):
    ...
    wordlist = [word for word in wordlist if word.lower()
...
def indexWebASite(url, urlSet, depth, urlAmount):
    ...
    for i in range(len(urlSet)):
        t = Thread(target=threading, args=(
            urlSet[i], depth, urlAmount, keywords))
        t.start()
        threads.append(t)
        threading(urlSet[i], depth, urlAmount, keywords)
...
def threading(url, depht, urlAmount, keywords):
    ...
    root = AnyNode(name='b', urlName=url, kwf=kwf)
    result = createKeywordFrequancyTree(
        root, root, depht, 0, keywords, url, urlAmount)
    resultArr.append(exporter._export(result))
...
def createKeywordFrequancyTree(root, parent, stoperIndex, deep, keywords, iterationUrl, urlAmount):
    ...
    for url in urlList:
        keywords2 = FindKeywords(url, 10)
        kwf = FindSimilarity(keywords, keywords2)['wordCounts']
        newParent = AnyNode(name='a', urlName=url, kwf=kwf, parent=parent)
        newDeep = deep+1
        createKeywordFrequancyTree(
            root, newParent, stoperIndex, newDeep, keywords, url, urlAmount)
    return root
